...
class UtilizatorControllerTest {
    ...
    @Test
    void existaPentruVanzator_true_returns200CuTrue() throws Exception {
        Mockito.when(utilizatorService.existaUtilizatorPentruVanzator(3L)).thenReturn(true);

        mockMvc.perform(get("/api/utilizatori/by-vanzator/3"))
                .andExpect(status().isOk())
                .andExpect(content().string("true"));
    }

    @Test
    void existaPentruVanzator_false_returns200CuFalse() throws Exception {
        Mockito.when(utilizatorService.existaUtilizatorPentruVanzator(3L)).thenReturn(false);
