...
class BonControllerTest {
    ...
    @Test
    void produsExistaPeBon_true_returns200CuTrue() throws Exception {
        Mockito.when(bonService.existaProdusPeVreunBon(5L)).thenReturn(true);

        mockMvc.perform(get("/api/bons/produse/5/pe-bon"))
                .andExpect(status().isOk())
                .andExpect(content().string("true"));
    }

    @Test
    void produsExistaPeBon_false_returns200CuFalse() throws Exception {
        Mockito.when(bonService.existaProdusPeVreunBon(5L)).thenReturn(false);
